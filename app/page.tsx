...
// Hook simple para saber si estamos en pantallas pequeñas/medianas
      ...
      if (!canceled) {
        ...
        (async () => {
          ...
          if (medioFromLink && medioFromLink.trim()) {
            ...
            setPage(1);
            // NO activamos fullscreen en ingresos por link compartido (según tu aclaración previa).
...
<body>
  ...
  const barColor = dark ? primaryDark : '#0ea5e9';

  // Ajustes responsivos para charts
  const barSizeMain = bp === 'sm' ? 28 : bp === 'md' ? 36 : 44;
  const yAxisWidth = bp === 'sm' ? 64 : 84;

  // Etiquetas según viewport
  const showYearLabels = bp !== 'sm';
  const showMonthLabels = bp === 'lg' || bp === 'xl';
    ...
    <div className={`${dark ? 'bg-neutral-900 text-neutral-100' : 'bg-neutral-50 text-neutral-900'}`}>
      ...
      <main className="mx-auto max-w-7xl px-3 sm:px-4 py-6 space-y-8">
        ...
        <section className={`card p-3 sm:p-4 ${dark ? 'bg-neutral-800 border border-neutral-700' : 'bg-white border border-neutral-200'}`}>
          ...
          <div className="mt-3 h-60 sm:h-72 w-full">
            <ResponsiveContainer width="100%" height="100%">
              {chartMode === 'yearTotals' ? (
                ...
                <BarChart data={totalsByMonthOfYear.rows} barSize={bp === 'sm' ? 24 : bp === 'md' ? 30 : 36} margin={{ top: 64, right: 12, bottom: 10, left: yAxisWidth }}>
                  <CartesianGrid vertical={false} strokeDasharray="3 3" stroke={dark ? '#374151' : '#cfeffd'} />
                  <XAxis dataKey="mes" stroke={dark ? '#d1d5db' : '#0b4b66'} interval={0} tick={{ fontSize: 11 }} />
                  <YAxis
                    width={yAxisWidth}
                    tick={{ fontSize: 12, fill: dark ? '#d1d5db' : '#0b4b66' }}
                    domain={[0, (dataMax: number) => (dataMax || 0) * 1.22]}
                    tickFormatter={v => currencyFormatter.format(v).replace(/^\$\s?/, '$')}
                    allowDecimals={false}
                  />
                  <Tooltip
        ...
        {/* Botones azules para desplegar Top 20 e Índice A–Z (cerrados por defecto) */}
        <section className={`card p-3 sm:p-4 ${dark ? 'bg-neutral-800 border border-neutral-700' : 'bg-white border border-neutral-200'}`}>
          {/* Botón Top 20 */}
          <button
        ...
        {/* Detalle (pantalla completa absoluta al abrir desde búsqueda/top/índice) */}
        <section
          ref={detailRef}
          className={
            `${detailFullscreen
        ...
        >
          {/* Barra superior del detalle: título + compartir + fullscreen */}
          ...
          {/* Gráfico por medio seleccionado (con selector de modo y año) */}
          {selectedMedium && (
            <div className="mb-4">
              ...
              <div className="h-52 sm:h-60 w-full">
                <ResponsiveContainer width="100%" height="100%">
                  {detailChartMode === 'years' ? (
                    <BarChart data={mediumTotalsByYear} barSize={bp === 'sm' ? 28 : 36} margin={{ top: 48, right: 16, bottom: 10, left: yAxisWidth }}>
                      <CartesianGrid vertical={false} strokeDasharray="3 3" stroke={dark ? '#374151' : '#cfeffd'} />
                    ...
                    <BarChart data={mediumTotalsMonthsOfYear.rows} barSize={bp === 'sm' ? 24 : 30} margin={{ top: 58, right: 16, bottom: 10, left: yAxisWidth }}>
                      <CartesianGrid vertical={false} strokeDasharray="3 3" stroke={dark ? '#374151' : '#cfeffd'} />
                      <XAxis dataKey="mes" stroke={dark ? '#d1d5db' : '#0b4b66'} interval={0} tick={{ fontSize: 11 }} />
                      <YAxis
                        width={yAxisWidth}
